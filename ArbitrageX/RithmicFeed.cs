...
{
    private const string SourceTerminalId = "RITHMIC-FAST";
    ...
    public RithmicFeed(TickRingBuffer tickBuffer)
    {
    ...
    private void ConnectInternal()
    {
        try
        {
            ...
            _callbacks = new RithmicCallbacks(_tickBuffer, ticker);
    ...
    private sealed class RithmicCallbacks : RCallbacks
    {
        ...
        private readonly int _terminalHash = HftHash.GetHash(SourceTerminalId);
        private readonly int _configuredSymbolHash;
        private readonly TickRingBuffer _tickBuffer;
        ...
        public RithmicCallbacks(TickRingBuffer tickBuffer, string configuredTicker)
        {
            _tickBuffer = tickBuffer;
            _configuredSymbolHash = HftHash.GetSymbolHash(configuredTicker ?? string.Empty);
        }
        ...
        private void EmitTickIfReady()
        {
            ...
            int symbolHash = _configuredSymbolHash != 0 ? _configuredSymbolHash : HftHash.GetSymbolHash(_lastSymbol);
            _tickBuffer.Write(_terminalHash, symbolHash, _lastBid, _lastAsk, ts);
            _emittedTicks++;
            EmitDiagnosticsIfDue(ts);
        }
